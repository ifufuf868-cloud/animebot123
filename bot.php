...
if ($text == "✉ Xabar Yuborish" and in_array($cid, $admin)) {
    ...
    if (!$row) {
        bot('sendMessage', [
            'chat_id' => $cid,
            'text' => "<b>ðŸ“¤ Foydalanuvchilarga yuboriladigan xabarni botga yuboring!</b>",
            'parse_mode' => 'html',
            'reply_markup' => $aort
        ]);
        ...
        bot('sendMessage', [
            'chat_id' => $cid,
            'text' => "<b>ðŸ“‘ Hozirda botda xabar yuborish jarayoni davom etmoqda. Yangi xabar yuborish uchun eski yuborilayotgan xabar barcha foydalanuvchilarga yuborilishini kuting!</b>",
            'parse_mode' => 'html',
            'reply_markup' => $panel
        ]);
        exit;
...
if ($text == "🚀 Konkurs" and in_array($cid, $admin)) {
    if ($konkurs == "on") {
        sms($cid, "$giveText\n-\n🟢Davom etmoqda...", json_encode(['inline_keyboard' => [
            [['text' => "🔴Konkursni tugallash", 'callback_data' => "endkonkurs"]],
            [['text' => "🗑Takliflar sonini tozalash", 'callback_data' => "delomdam"]]
        ]]));
        exit();
    } else {
        sms($cid, "Konkursni tugatildi!", json_encode(['inline_keyboard' => [
            [['text' => "🟢Konkursni boshlash", 'callback_data' => "startkonkurs"]],
            [['text' => "🗑Takliflar sonini tozalash", 'callback_data' => "delomdam"]]
        ]]));
        exit();
    }
...
if ($data == "startkonkurs" and in_array($cid2, $admin)) {
    del();
...
if ($step == "startkonkurs" and in_array($cid, $admin)) {
    file_put_contents("admin/konkurs_text.txt", $text);
    file_put_contents("admin/konkurs.txt", "on");
    sms($cid, "✅Konkurs boshlandi !", $panel);
    unlink("step/$cid.step");
    exit();
}

if ($data == "endkonkurs" and in_array($cid2, $admin)) {
    edit($cid2, $mid2, "<b>⚠️Konkursni to'xtatishga isonchingiz komilmi?</b>", json_encode(['inline_keyboard' => [
        [['text' => "✅Ha", 'callback_data' => "hakonkurs"], ['text' => "❌Yo'q", 'callback_data' => "boshqarish"]]
    ]]));
}

if ($data == "hakonkurs" and in_array($cid2, $admin)) {
    file_put_contents("admin/konkurs.txt", "off");
    del();
    sms($cid2, "✅Konkurs to'xtatildi!", $panel);
}

if ($data == "delomdam" and in_array($cid2, $admin)) {
